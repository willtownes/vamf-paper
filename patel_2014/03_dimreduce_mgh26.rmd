...
dir.create("../biostat_paper/data/patel",recursive=TRUE)
...
```{r}
factors<-existing$cidr(Y_raw[g2k,],2)
patel_plt(factors,meta)
pd<-cbind(factors,det_rate=meta$detection_rate,instrument=meta$instrument)
round(cor(pd[,-4]),2) #.42 correlation
ggplot(pd,aes(x=det_rate,y=dim1,colour=instrument,shape=instrument))+geom_point(size=3)+theme_bw()+theme(legend.position="element_blank")+xlab("detection rate")+ylab("CIDR1")
```
...
dat[["CIDR"]]<-lapply(Ysubsets_raw,existing$cidr)
...
#plt1_methods<-c("CIDR","tSNE_plx_5","tSNE_plx_20","PCA")
