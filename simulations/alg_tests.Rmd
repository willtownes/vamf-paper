...
### Regression against Detection Rates followed by PCA on Residuals

```{r}
dat[["regpca"]]<-factors<-existing$regpca(Y_obs,2)
...
dat[["CIDR"]]<-factors<-as.data.frame(sData@PC[,c(1,2)])
...
```{r}
dat[["regpca"]]<-factors<-existing$regpca(Y_obs,2)
plt_clst(factors,ref)+ggtitle("Regression-PCA")
ggs("regpca.pdf")
```
